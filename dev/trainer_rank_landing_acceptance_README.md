...
| dense attention classes (`dev/trainer_rank_cost_calibration_lattice.sky.yaml`: Qwen3-1.7B, Qwen3-8B, Qwen3-14B; Qwen3-4B real data at CP4/TP2×CP2) | 4x H200 | same protocol on the Qwen3-tokenized Ellavox groups and synthetic families at TP1 × CP1/2/4, TP2 × CP1, TP2 × CP2 (70 cells each) | certified at the shapes whose gates pass: `dense-attn-h2048`/`h4096` at CP1, CP2, TP2 (42 cells; pairwise 96.5% / 99.9%, max regret 1.0% / 1.1%), `dense-attn-h5120` also at TP2 × CP2 (56 cells; 99.9%, max 1.0%); TP1 × CP4 (and TP2 × CP2 for the two smaller classes) measured but not admitted — the ten terms cannot see the CP exchange schedule (held-out max regret 26% / 14% / 17%) — and the dense table withholds Qwen3-4B × TP1 × CP4 (35% vs version 1's 15%); CP-aware re-ranker planned (design brief) |
| `dev/trainer_rank_cost_calibration_certificate_<table>.json` (one per calibrated table, compact: one line per cell) + `tests/unit/test_planner_cost_certificate.py` | CPU | the shipped table is the certified table (hash) and the certified metrics hold on the recorded per-cell aggregates; exact cell identities match each table's manifest (dense: the 58 fitted cells plus the 16 excluded blind-spot cells; attention classes: their CP4 and failing TP2 × CP2 cells excluded with reasons) and each table's withheld pairs are exactly the manifest's; the admitted execution classes (device classes, dtypes, model geometries, parallel shapes) equal the certified cells' and every geometry was measured at every admitted shape; runners fail loudly on any cell failure and the fitter refuses incomplete evidence unless exclusions are explicit (the two Ellavox CP4 cells that hung in the campaign were a harness desynchronization, issue #840, fixed and re-measured) | pass |
