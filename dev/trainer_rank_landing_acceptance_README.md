...
| dense attention classes (`dev/trainer_rank_cost_calibration_lattice.sky.yaml`: Qwen3-1.7B, Qwen3-8B, Qwen3-14B; Qwen3-4B real data at CP4/TP2×CP2) | 4x H200 | same protocol on the Qwen3-tokenized Ellavox groups and synthetic families at TP1 × CP1/2/4, TP2 × CP1, TP2 × CP2 (70 cells each); candidate rows record the CP plan structure (`cp_plan`: wave count, max rank tokens, summary cost) and the version-1 score | `dense-attn-h2048`/`h4096` at CP1, CP2, TP2 (42 cells; pairwise 96.5% / 99.9%, max regret 1.0% / 1.1%), `dense-attn-h5120` also at TP2 × CP2 (56 cells; 99.9%, max 1.0%); TP1 × CP4 admitted through the two-stage re-ranker (`--rerank-shapes tp1cp4`: shortlist of 3 + depth-one anchor, re-rank by waves and max rank load; two-stage max regret 2.6% / 0.8% / 4.1%, every clear winner shortlisted, no >2% regression vs cheap or version 1, planning cost mean 2.6% / 1.6% / 1.0% of cell time below the mean saving); TP2 × CP2 not admitted for the two smaller classes; the dense table withholds Qwen3-4B × TP1 × CP4 (35% vs version 1's 15%) |
| Qwen3-30B-A3B class (`dev/trainer_rank_cost_calibration_lattice.sky.yaml`, EP1 at CP1/2/4/TP2, EP2 at CP2/CP4/TP2, EP4 at CP4) | 4x H200 | 112 cells on the Qwen3-tokenized Ellavox groups and synthetic families; 3 CP1 cells fail single-GPU memory admission (#848) | `attn-moe-h2048-h200-bf16`: direct at CP1, TP2, TP2 × EP2 (39 cells; 99.9%, max 2.2%), CP4 × EP2 through the re-ranker (max 2.1%); CP2 (EP1/EP2) and CP4 (EP1/EP4) measured but not admitted — sporadic 10–20 s stalls on ~4% of rounds and drift fail both selections' gates there; sixteen-round re-measurement queued |
| `dev/trainer_rank_cost_calibration_certificate_<table>.json` (one per calibrated table, compact: one line per cell) + `tests/unit/test_planner_cost_certificate.py` | CPU | the shipped table is the certified table (hash) and the certified metrics hold on the recorded per-cell aggregates; exact cell identities match each table's manifest (dense: the 58 fitted cells plus the 16 excluded blind-spot cells; attention classes: their CP4 and failing TP2 × CP2 cells excluded with reasons) and each table's withheld pairs are exactly the manifest's; tables with a re-ranker: the shipped re-ranker equals the certificate's `reranker` block and the two-stage selection recomputed from the recorded aggregates reproduces its metrics and passes the gates on every re-ranked shape; the admitted execution classes (device classes, dtypes, model geometries, parallel shapes) equal the certified cells' and every geometry was measured at every admitted shape; runners fail loudly on any cell failure and the fitter refuses incomplete evidence unless exclusions are explicit (the two Ellavox CP4 cells that hung in the campaign were a harness desynchronization, issue #840, fixed and re-measured) | pass |
