...
| dense controls (`dev/trainer_rank_cost_calibration_lattice.sky.yaml`: Qwen3-1.7B, Qwen3-8B, Qwen3-4B real data at CP4/TP2×CP2) | 4x H200 | same protocol on the Qwen3-tokenized Ellavox groups and synthetic families at TP1 × CP1/2/4, TP2 × CP1, TP2 × CP2 | not certified: the ten terms fail the gates at CP4 (held-out max regret 26% / 14%) because exchange-wave count and rank imbalance of the CP plan are not layout features; the shipped dense table shows a 35% regret cell on Qwen3-4B real data at TP1 × CP4 (known limitation in the design brief) |
| `dev/trainer_rank_cost_calibration_certificate_<table>.json` (one per calibrated table, compact: one line per cell) + `tests/unit/test_planner_cost_certificate.py` | CPU | the shipped table is the certified table (hash) and the certified metrics hold on the recorded per-cell aggregates; exact 58 cell identities match the manifest with no exclusions; the admitted execution classes (device classes, dtypes, model geometries, parallel shapes) equal the certified cells' and every geometry was measured at every admitted shape; runners fail loudly on any cell failure and the fitter refuses incomplete evidence unless exclusions are explicit (the two Ellavox CP4 cells that hung in the campaign were a harness desynchronization, issue #840, fixed and re-measured) | pass |
| `tests/unit/test_planner_cost_profile.py` | CPU | a fitted table applies only to the execution classes it was measured on (device class, dtype, Megatron-config model geometry, TP × CP × EP × ETP shape), bound to its certificate by test; outside every table the version-1 score is used verbatim, unmeasured shapes such as TP2 × CP2 included | pass |
