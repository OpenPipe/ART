...
async def rollout(client: openai.AsyncOpenAI, prompt: str, model_name: str) -> art.Trajectory:
...
async def main():
    ...
    for step in range(start_step, 1_000):
        ...
        train_groups = await art.gather_trajectory_groups(
            (
                art.TrajectoryGroup(rollout(openai_client, prompt, model.name) for _ in range(32))
