...
from art.trainer_rank import AdamParams, ForwardInput, ForwardOutput, TrainerRank, _impl
from art.trainer_rank._impl import _CheckpointSlot, _GatherContextParallelRows
from art.trainer_rank._rng import TrainerRNG, _State, caller_group
...
def test_model_stream_advances_without_advancing_caller(monkeypatch):
    ...
    trainer.dp_rank_forward([], no_grad=True)
...
def test_microbatch_yields_and_close_do_not_hold_rng_context(monkeypatch, yield_empty):
    ...
    monkeypatch.setattr(trainer, "_forward_micro_batches", batches)
    iterator = trainer.forward_micro_batches([], yield_empty=yield_empty)
    next(iterator)
...
def _gradient_oracle(
    ...
    patch.setattr(trainer, "_forward_micro_batches", batches)
    for step in range(2):
        losses = []
        expected_losses = []
        for micro in range(2):
            ...
            before = _State.capture(device)
            if step == 0:
                output = trainer.dp_rank_forward([request])[0]
            else:
                iterator = trainer.forward_micro_batches([request])
                output = next(iterator).outputs[0]
                assert trainer._rng._depth == 0
                iterator.close()
            if replica_rank == 0:
                _assert_state_equal(_State.capture(device), before)
            caller = _State.capture(device)
        ...
        before_backward = _State.capture(device)
        tracker_before = tracker.get_states() if tracker is not None else {}
        torch.stack(losses).sum().backward()
        _assert_state_equal(_State.capture(device), before_backward)
        if tracker is not None:
