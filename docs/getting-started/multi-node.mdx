...
the controller. `examples/multinode/program.py` is a complete CPU-runnable
Yes/No/Maybe smoke:
...
If `ART_VLLM_RUNTIME_BIN` is a custom command wrapper, also set
`vllm_probe_command` to a Python-compatible command prefix in
`NcclTransportSpec`. ART uses that typed command to run the collective in the
same vLLM environment; the command must accept Python's `-c <script>` form.
