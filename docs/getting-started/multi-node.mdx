...
For a HybridEP deployment, each host can materialize and validate the exact
cross-host runtime before allocating the model:

```bash
art runtime prepare --hybrid-ep --multinode
art runtime status --hybrid-ep --multinode
```

Keep `/tmp/art-cache`, or an explicitly configured runtime cache, on local SSD.
Use the shared mount for checkpoints and run artifacts. ART reports the selected
CUDA profile and warns when a runtime cache resolves to a network filesystem.
