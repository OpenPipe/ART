...
The ART server can be run locally on any machine with a GPU. If you don't have one, the [SkyPilot integration](/integrations/skypilot-integration) can provision one on Kubernetes or any major cloud. To install the backend dependencies required for training and inference, you can install the `backend` extra:
...
You can install and validate both pinned GPU runtimes before starting a model:
...
`prepare` reports each dependency, runtime, and validation phase with elapsed
time. `status` shows the selected CUDA profile, active runtime paths, cache disk
use, and free space. Model-specific vLLM graph and kernel compilation still
happens when that model first starts.

Runtime environments and compiler outputs default to `/tmp/art-cache`. Keep
this cache on fast node-local storage; ART warns when it detects a network
filesystem. Set `ART_MEGATRON_CACHE_ROOT` to move the complete model/runtime
cache, or set `ART_MEGATRON_RUNTIME_CACHE_DIR` and
`ART_VLLM_RUNTIME_CACHE_DIR` individually. Keep durable checkpoints on shared
storage rather than moving runtime environments there.
