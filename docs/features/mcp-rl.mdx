...
- W&B API key for serverless inference and training
- OpenRouter API key for scenario generation and judging
- Python environment with ART main (pinned to `88d3df2`) installed
...
model = art.TrainableModel(
    name="mcp-agent-qwen38",
    run_name="mcp-agent-qwen38",
    project="mcp-rl",
    base_model="Qwen/Qwen3.8-27B",
)
backend = ServerlessBackend()  # Uses WANDB_API_KEY
...
scenario_collection = await generate_scenarios(
    ...
    num_scenarios=100,
    show_preview=False,
    generator_model="openai/gpt-4o-mini",  # Uses OPENROUTER_API_KEY
)
...
groups = await gather_trajectory_groups(
    (
        ...
        for scenario in scenario_collection.scenarios
    ),
    pbar_desc="train gather step",
)
