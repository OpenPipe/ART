...
- Provision a remote machine with an advanced GPU (by default on RunPod)
- Install `openpipe-art` and its dependencies
- Initialize a `LocalBackend` instance with vLLM and a training server (unsloth or torchtune)
- Register the `LocalBackend` instance to forward requests to it over http
