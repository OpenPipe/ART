...
**Supervised fine-tuning (SFT)** trains a model on labeled chat examples rather than through trial-and-error with rewards. It's useful for **response-level distillation** (training on outputs sampled from a teacher), **teaching a specific output style or format**, and **warming up** a model before RL training so it starts from a stronger baseline.
...
Response-level distillation trains a student on completions sampled from a teacher. It is ordinary SFT: generate responses from the teacher, wrap them as trajectories, and fine-tune. To train on token-level teacher distributions with OPD or OPSD instead, see [On-Policy Distillation](/experimental/on-policy-distillation).
