"""ART-owned Qwen3.5 chat template defaults."""

QWEN3_5_DISABLE_THINKING_MULTI_SYSTEM_CHAT_TEMPLATE = r"""{%- set image_count = namespace(value=0) %}
...
{%- if tools and tools is iterable and tools is not mapping %}
    ...
    {%- if messages[0].role == 'system' %}
        ...
        {%- if content %}
            {{- '\n\n' + content }}
        {%- endif %}
        {%- set ns.consumed_first_system = true %}
    {%- endif %}
    {{- '<|im_end|>\n' }}
{%- endif %}
...
{%- for message in messages %}
    {%- if ns.consumed_first_system and loop.first and message.role == 'system' %}
