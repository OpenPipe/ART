...
class DeepseekV4RotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(
        ...
        """
        base = config.rope_parameters[layer_type]["rope_theta"]  # ty:ignore[invalid-argument-type, possibly-missing-attribute]
        # key difference to gemma3: partial rope
        partial_rotary_factor = config.rope_parameters[layer_type].get(  # ty:ignore[invalid-argument-type, possibly-missing-attribute]
            "partial_rotary_factor", 1.0
        )  # ty:ignore[invalid-argument-type, possibly-missing-attribute]
        head_dim = (
            getattr(config, "head_dim", None)
            or config.hidden_size // config.num_attention_heads  # ty:ignore[possibly-missing-attribute]
...
class DeepseekV4HCACache(DynamicSlidingWindowLayer):
    ...
    def update(
        ...
        full = torch.cat([self.keys, key_states], dim=-2)  # ty:ignore[no-matching-overload]
    ...
    def store_compression_weights(
        ...
        if buffered_kv is not None and buffered_kv.shape[1]:
            ...
            gate = torch.cat([buffered_gate, gate], dim=1)  # ty:ignore[no-matching-overload]
    ...
    def update_compressor_states(
        self, name: str, compressed: torch.Tensor
    ) -> torch.Tensor:
        r"""
        ...
        return self.compressed_kv[name]  # ty:ignore[invalid-return-type]
...
class DeepseekV4GroupedLinear(nn.Linear):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # ty:ignore[invalid-method-override]
    ...
    def forward(
        ...
        if chunk_kv.shape[1] > 0:  # there were at least self.compress_rate tokens
            ...
            chunk_gate = chunk_gate.view(
                batch, n_windows, self.compress_rate, -1
            ) + self.position_bias.to(chunk_gate.dtype)
            compressed = self.kv_norm(
                (
                    chunk_kv
                    * chunk_gate.softmax(dim=2, dtype=torch.float32).to(chunk_kv.dtype)
                ).sum(dim=2)
            )
        ...
        if seq_len == 1 or compressed_len == 0:
            return compressed_kv, None  # ty:ignore[invalid-return-type]
...
class DeepseekV4Indexer(nn.Module):
    ...
    def forward(
        ...
        if chunk_kv.shape[1] > 0:
            ...
            if cache_layer is not None:
                ...
                if prior_kv is not None:
                    ...
                    new_gate[:, 0, :ratio] = prior_gate.to(new_gate.dtype)  # ty:ignore[possibly-missing-attribute]

            compressed = self.kv_norm(
                (
                    new_kv
                    * new_gate.softmax(dim=2, dtype=torch.float32).to(new_kv.dtype)
                ).sum(dim=2)
            )
            positions = torch.arange(n_windows, device=compressed.device)
        ...
        weights = (
            self.weights_proj(hidden_states).float() * self.weights_scaling
        )  # [B, S, H]
...
class DeepseekV4CSACompressor(nn.Module):
    ...
    def forward(
        ...
        return compressed_kv, block_bias[..., :compressed_len]  # ty:ignore[invalid-return-type]
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    key_states = repeat_kv(key, module.num_key_value_groups)  # ty:ignore[invalid-argument-type]
    value_states = repeat_kv(value, module.num_key_value_groups)  # ty:ignore[invalid-argument-type]
    attn_weights = torch.matmul(query, key_states.transpose(2, 3)) * scaling
    if attention_mask is not None:
        attn_weights = attn_weights + attention_mask

    sinks = module.sinks.reshape(1, -1, 1, 1).expand(  # ty:ignore[call-non-callable]
...
class DeepseekV4Attention(nn.Module):
    ...
    * 3 different cache mechanisms, sliding, sliding+CSA, sliding+HCA.
    ...
    def __init__(self, config: DeepseekV4Config, layer_idx: int):
        ...
        )  # single KV head, broadcast to all
        self.head_dim = config.head_dim
        ...
        self.compressor = (
            ...
            else None  # ty:ignore[call-non-callable]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        cos, sin = position_embeddings[self.rope_layer_type]  # ty:ignore[invalid-argument-type]
        ...
        if (
            ...
            and kv.shape[2] > attention_mask.shape[-1]
        ):
            if block_bias is not None:
                ...
                attention_mask = F.pad(
                    attention_mask,
                    (0, kv.shape[2] - attention_mask.shape[-1]),
                    value=0.0,
                )
...
class DeepseekV4HashRouter(nn.Module):
    ...
    def forward(
        ...
        logits = F.linear(flat, self.weight)
        scores = self.score_fn(logits)
        indices = self.tid2eid[input_ids.reshape(-1)].long()  # ty:ignore[not-subscriptable]
        weights = scores.gather(1, indices)
        weights = weights / (weights.sum(dim=-1, keepdim=True) + 1e-20)
        return logits, weights * self.routed_scaling_factor, indices
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if position_ids is None:
            position_ids = cache_position  # ty:ignore[invalid-assignment]
            position_ids = position_ids.unsqueeze(0)  # ty:ignore[invalid-assignment, possibly-missing-attribute]
...
def load_balancing_loss_func(
    ...
    expert_mask = torch.nn.functional.one_hot(selected_experts, num_experts)  # ty:ignore[invalid-argument-type]
    ...
    if attention_mask is None:
        ...
        router_per_expert_attention_mask = (
            ...
            .expand((num_hidden_layers, batch_size, sequence_length, num_experts))  # ty:ignore[no-matching-overload]
...
class DeepseekV4ForCausalLM(DeepseekV4PreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(cls, *args, **kwargs):
        ...
        enable_source_aliases = config is not None and bool(
            getattr(config, "dsv4_oracle_source_aliases", False)
        )
        if enable_source_aliases:
            setattr(config, "dsv4_oracle_source_aliases", False)
        try:
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if output_router_logits:
            ...
            if labels is not None:
                loss += (
                    self.router_aux_loss_coef * aux_loss.to(loss.device)  # ty:ignore[possibly-missing-attribute]
                )  # make sure to reside in the same device  # ty:ignore[possibly-missing-attribute]

        return MoeCausalLMOutputWithPast(
            loss=loss,
            aux_loss=aux_loss,  # ty:ignore[invalid-argument-type]
            logits=logits,
