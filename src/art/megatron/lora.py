...
class ComponentwiseColumnParallelLinearLoRA(torch.nn.Module):
    """One LoRA pair for a column projection whose output packs TP-sharded components."""
    ...
    def __init__(
        ...
        super().__init__()
        components = tuple(int(size) for size in component_sizes)
        if not components or any(size <= 0 for size in components):
            raise ValueError(f"Invalid component sizes: {components}")
        tp_size = int(getattr(in_proj, "tp_size", _get_shard_world_size("tp")))
        if any(size % tp_size for size in components):
            raise ValueError(
                f"Component sizes {components} must be divisible by TP size {tp_size}"
            )
        weight = getattr(in_proj, "weight", None)
        if not isinstance(weight, torch.Tensor):
            raise TypeError(f"{type(in_proj).__name__}.weight must be a tensor")
        local_component_sizes = tuple(size // tp_size for size in components)
        if sum(local_component_sizes) != int(weight.shape[0]):
            raise ValueError(
                f"Component sizes {components} imply local width "
                f"{sum(local_component_sizes)}, got {int(weight.shape[0])}"
            )
        partition_sizes = getattr(weight, "partition_sizes", None)
        if (
            partition_sizes is not None
            and tuple(partition_sizes) != local_component_sizes
        ):
            raise ValueError(
                f"Projection partition sizes {tuple(partition_sizes)} do not match "
                f"component layout {local_component_sizes}"
            )
        if isinstance(in_proj, TELayerNormColumnParallelLinear):
            in_proj.return_layernorm_output = True
            in_proj.return_layernorm_output_gathered = True
        self.in_proj = in_proj
        self.component_sizes = components
        self.lora = _parallel_lora(
            adapter_model_prefix=adapter_model_prefix,
            linear=in_proj,
            out_features=sum(local_component_sizes),
            rank=rank,
            alpha=alpha,
            layout="column",
            lora_cls=lora_cls,
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor | None]:
        ...
        if adapter.shape != base.shape:
            raise RuntimeError(
                f"{self.lora.adapter_model_prefix}: LoRA adapter output shape "
                f"{tuple(adapter.shape)} does not match base output shape {tuple(base.shape)}"
            )
...
def wrap_split_mlp_lora(
    ...
    *,
    ...
    if _targets_include(
        target_modules,
        *(("up_proj",) if non_gated else ("gate_proj", "up_proj")),
    ):
