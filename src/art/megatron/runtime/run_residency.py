...
class RunResidencyPolicy(BaseModel):
    """Per-rank trainer residency limits resolved on the deployed GPU host."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    l1_gpu_max_bytes: int | None = Field(default=None, ge=1)
    l2_cpu_max_bytes: int = Field(default=64 << 30, ge=1)
    l3_nvme_max_bytes: int = Field(default=256 << 30, ge=1)
    nvme_root: str | None = Field(default=None, min_length=1)
    runtime_free_floor_bytes: int = Field(default=8 << 30, ge=0)
    max_concurrent_transitions: int = Field(default=2, ge=1, le=8)

    @model_validator(mode="after")
    def _validate_explicit_l1(self) -> "RunResidencyPolicy":
        if self.l1_gpu_max_bytes is not None and (
            self.l2_cpu_max_bytes < self.l1_gpu_max_bytes
            or self.l3_nvme_max_bytes < self.l1_gpu_max_bytes
        ):
            raise ValueError("L2 and L3 must each hold the complete L1 budget")
        return self
    ...
    def resolve(
        ...
        *,
        ...
        l1_bytes = self.l1_gpu_max_bytes or _default_l1_bytes(
            device_name, device_total_bytes
        )
        if self.l2_cpu_max_bytes < l1_bytes or self.l3_nvme_max_bytes < l1_bytes:
            raise ValueError("resolved L2 and L3 must each hold the complete L1 budget")
        return RunResidencyConfig(
            limits=ResidencyLimits(
                l1_gpu=TierCapacity(max_bytes=l1_bytes),
                l2_cpu=TierCapacity(max_bytes=self.l2_cpu_max_bytes),
                l3_nvme=TierCapacity(max_bytes=self.l3_nvme_max_bytes),
                max_concurrent_transitions=self.max_concurrent_transitions,
            ),
            nvme=NvmeResidencyStoreConfig(
                root=root,
                shared_storage_mount=mount,
                runtime_free_floor_bytes=self.runtime_free_floor_bytes,
            ),
        )


def _default_l1_bytes(device_name: str, device_total_bytes: int) -> int:
    normalized = device_name.upper()
    if "B300" in normalized:
        return 24 << 30
    if "H200" in normalized:
        return 16 << 30
    if device_total_bytes < 4 << 30:
        raise ValueError("trainer GPU is too small for managed run residency")
    return min(16 << 30, device_total_bytes // 4)
