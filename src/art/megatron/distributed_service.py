...
from .lora_config import LORA_ALPHA
...
class DistributedMegatronService:
    ...
    def _runtime_spec(self) -> TrainerRuntimeSpec:
        return build_trainer_runtime_spec(
            self.runtime,
            base_model=self.base_model,
            config=self.config,
            enable_expert_replay=self.enable_expert_replay,
    ...
    async def _ensure_trainer_locked(
        ...
        self._trainer = await self._launch_trainer(current, run_id=run_id)
    ...
    async def _launch_trainer(self, current: str, *, run_id: str | None = None) -> Any:
