"""Topology-portable persistence for Megatron LoRA run checkpoints."""
...
class CheckpointSlot:
    ...
    optimizer: DynamicOptimizer | None = None
    revision: int = 0
...
def _optimizer_config(dynamic: DynamicOptimizer) -> OptimizerConfig:
...
def _validate_save_state(trainer: CheckpointHost, name: str) -> dict[str, object]:
...
def _custom_snapshot(
    ...
    if not slot.custom and slot.custom_payload is None:
        return {}
...
def prepare_checkpoint_save(
    trainer: CheckpointHost, output_dir: str, checkpoint_name: str
) -> None:
...
def _finish(trainer: CheckpointHost, prepared: _PreparedSave) -> None:
...
def _advance_save_queue(trainer: CheckpointHost, sequence: int) -> None:
    with trainer._checkpoint_save_condition:
...
def _record_finalized_save(
    trainer: CheckpointHost, output_dir: str, finalized: _FinalizedSave
) -> None:
...
def finish_checkpoint_save(trainer: CheckpointHost, output_dir: str) -> None:
    _finalize_checkpoint_save(trainer, output_dir, "finish")


def abort_checkpoint_save(trainer: CheckpointHost, output_dir: str) -> None:
...
def _slot_snapshot(trainer: CheckpointHost) -> _SlotSnapshot:
...
def _commit_slot(trainer: CheckpointHost, source: str, destination: str) -> None:
    from art.megatron.lora import LoRA
...
def _rollback_load(
    ...
    def rollback() -> None:
        ...
        if previous is None:
            ...
            trainer._checkpoint_slots[name] = cast(CheckpointSlot, previous)
...
def load_checkpoint(
    ...
    try:
        ...
        trainer._checkpoint_slots[temporary] = CheckpointSlot(
...
def _ensure_group(trainer: CheckpointHost) -> dist.ProcessGroup | None:
    return _ensure_groups(trainer)[0]


def _ensure_finalize_group(trainer: CheckpointHost) -> dist.ProcessGroup | None:
