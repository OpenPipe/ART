...
def _find_nccl_library() -> str:
    ...
    if torch.version.cuda is not None:
        ...
        if spec is None or spec.submodule_search_locations is None:
            raise RuntimeError(
                "CUDA weight transfer requires the matching nvidia-nccl-cu12 or nvidia-nccl-cu13 package."
            )
        nccl_library = (
            Path(next(iter(spec.submodule_search_locations))) / "lib" / "libnccl.so.2"
        )
        if not nccl_library.exists():
            raise RuntimeError(f"The NVIDIA NCCL package is missing {nccl_library}")
