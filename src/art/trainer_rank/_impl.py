...
# ``_slots`` and ``_dynamic_optimizer`` hold TrainerRank method bodies; ``_slots``
# reads ``_impl`` names such as ``Unset`` at definition time, so they are imported
# once those exist and before the class body binds their functions. The optimizer
# module is aliased because ``TrainerRank._dynamic_optimizer`` is also a method.
from art.trainer_rank import _dynamic_optimizer as _dynamic_optimizer_module
from art.trainer_rank import _slots
...
class TrainerRank:
    ...
    # Dynamic-optimizer management lives in ``_dynamic_optimizer`` (imported
    # as ``_dynamic_optimizer_module`` because a method shares its name);
    # binding the functions here keeps ``self._x(...)`` dispatch and
    # per-instance overrides (tests monkeypatch these) behaving as before.
    _extend_dynamic_optimizer = _dynamic_optimizer_module._extend_dynamic_optimizer
    optim_step = _dynamic_optimizer_module.optim_step
    _guard_optim_step_configuration = (
        _dynamic_optimizer_module._guard_optim_step_configuration
    )
    _dynamic_optim_step = _dynamic_optimizer_module._dynamic_optim_step
    _dynamic_param_step_flags = _dynamic_optimizer_module._dynamic_param_step_flags
    _dynamic_optimizer = _dynamic_optimizer_module._dynamic_optimizer
    _new_dynamic_optimizer = _dynamic_optimizer_module._new_dynamic_optimizer
    _restore_canonical_optimizer = (
        _dynamic_optimizer_module._restore_canonical_optimizer
    )
    _zero_dynamic_optimizer_padding = (
        _dynamic_optimizer_module._zero_dynamic_optimizer_padding
    )
    _dynamic_optimizer_padding_masks = (
        _dynamic_optimizer_module._dynamic_optimizer_padding_masks
    )
