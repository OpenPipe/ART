...
# ``_micro_batch_planner`` holds TrainerRank method bodies and reads ``_impl``
# names such as ``Unset`` at definition time, so it is imported once those exist
# and before the class body binds its functions.
from art.trainer_rank import _micro_batch_planner
