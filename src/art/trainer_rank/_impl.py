...
class TrainerRank:
    ...
    def load_checkpoint_sync(self, checkpoint: MaterializedCheckpoint) -> None:
        """Collectively replace one resident checkpoint from rank-local storage."""
        if self._slot_stack:
            raise RuntimeError("Cannot load a checkpoint while one is pushed")
        predecessor = self._checkpoint_mutation_tail
        if predecessor is not None:
        ...
        return self._step_dynamic_optimizer(
            (
                (
                    ...
                    model_params,
                    tuple(grads),
                    self._dynamic_param_step_flags(model_params),
                ),
            ),
            params=params,
        )
    ...
    def checkpoint_slot_optimizer_state(
        ...
        return {
            **state,
            "master_params": tuple(
                param.detach().cpu().clone() for param in state["master_params"]
            ),
            "optimizer": cast(dict[str, object], _state_to_cpu(state["optimizer"])),
        }
    ...
    def unload_checkpoint_slot(self, name: str) -> int:
        if self._slot_stack:
            raise RuntimeError("Cannot unload a checkpoint while one is pushed")
        slot = self._checkpoint_slots.get(name)
        if slot is None:
            raise ValueError(f"Unknown checkpoint slot: {name!r}")
        self._guard_slot_can_load(self._slot_ref(name))
        ...
        self._checkpoint_slots.pop(name)
        self._slot_graphs().pop(ref, None)
        if self._default_slot_ref == ref:
            self._default_slot_ref = None
        return unloaded
    ...
    def _restore_dynamic_optimizer(
        ...
        masters = state.get("master_params")
        optimizer_state = state.get("optimizer")
        if not isinstance(masters, Sequence) or not isinstance(
            optimizer_state, Mapping
        ):
            raise TrainerRankSlotStateError(
                f"Optimizer state for checkpoint slot {name!r} is incomplete."
        ...
        dynamic = self._new_dynamic_optimizer(
            ...
            master_params=cast(Sequence[torch.Tensor], masters),
        )
        try:
            dynamic.optimizer.load_state_dict(dict(optimizer_state))
        except ValueError as exc:
            raise TrainerRankSlotStateError(
                f"Optimizer state for checkpoint slot {name!r} does not match "
            ...
            for state_name, value in dynamic.optimizer.state.get(param, {}).items():
                if (
                    ...
                    raise TrainerRankSlotStateError(
                        ...
                        f"{name!r} has shape {tuple(value.shape)}, expected "
                        f"{tuple(param.shape)}."
                    )
        self._zero_dynamic_optimizer_padding(name, dynamic)
        return dynamic
    ...
    def _dynamic_optimizer_layout(self, name: str) -> TrainerRankOptimizerLayout:
        ...
        names_by_id = {
            id(param): names
            for names, param in iter_lora_slot_named_parameters(
                self.runtime.model, self._slot_ref(name)
            )
        }
        if any(id(param) not in names_by_id for param in params):
            raise TrainerRankSlotStateError(
