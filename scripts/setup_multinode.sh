...
core_prefix="/usr/local/nixl-${nixl_version}-cu${cuda_major}-${nixl_wheel_sha256:0:12}-layout2"
core_marker="nixl=${nixl_version} commit=${nixl_commit} wheel_sha256=${nixl_wheel_sha256} cuda=${cuda_version} layout=2"
core_stage="$build_root/nixl-core-stage"
install -d "$core_stage/include/gpu/ucx" "$core_stage/lib/x86_64-linux-gnu" "$core_stage/lib/nixl_cu${cuda_major}.libs" "$core_stage/python"
cp -a "$wheel_core/." "$core_stage/lib/x86_64-linux-gnu/"
cp -a "$wheel_deps/." "$core_stage/lib/nixl_cu${cuda_major}.libs/"
cp -a "$build_root/nixl-wheel/." "$core_stage/python/"
